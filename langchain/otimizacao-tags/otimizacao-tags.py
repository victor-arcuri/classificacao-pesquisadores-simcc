...
class Log:
    ...
    def save_log(self, tags: LogTag, groups: LogTagGroup):
        log = LogSchema(db_tags=tags, groups=groups)
...
class LogAgent():
    ...
    def create_new_log(self) -> Log:
        newLogPath = self.logsPath.joinpath(self.current_timestamp()) 
        os.mkdir(newLogPath)
        file = open(newLogPath.joinpath("log.json"), 'w')
        file.close()
        log = Log(newLogPath.joinpath("log.json"))
        return log
...
def logTest():
    ...
    groups = []
    log.save_log(tags=tags, groups=groups)
